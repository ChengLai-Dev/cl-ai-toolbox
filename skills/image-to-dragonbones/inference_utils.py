...
COMPACT_BODY_DROP = ['tail', 'wings', 'objects']

COMPACT_HEAD_MERGE = {
    'face': ['face', 'irides', 'eyebrow', 'eyewhite', 'eyelash', 'nose', 'mouth'],
}
COMPACT_HEAD_KEEP = ['headwear', 'eyewear']
...
def apply_layerdiff(
    ...
    if tag_version == 'v2':
        ...
        for rst, tag in zip(pipeline_output.images, body_tag_list):
            if taglist_mode == 'compact' and tag in COMPACT_BODY_DROP:
                continue
            savename = osp.join(saved, f'{tag}.png')
            Image.fromarray(rst).save(savename)
        head_img = images[2]
        ...
        if taglist_mode == 'compact':
            tag_to_merged = {}
            for merged_name, src_tags in COMPACT_HEAD_MERGE.items():
                for st in src_tags:
                    tag_to_merged[st] = merged_name
            merge_buffers = {k: [] for k in COMPACT_HEAD_MERGE}
            ...
            for rst, tag in zip(pipeline_output.images, head_tag_list):
                ...
                if tag in tag_to_merged:
                    merge_buffers[tag_to_merged[tag]].append(full.copy())
                elif tag in COMPACT_HEAD_KEEP:
                    savename = osp.join(saved, f'{tag}.png')
                    Image.fromarray(full).save(savename)

            for merged_name, buf in merge_buffers.items():
                if buf:
                    merged = img_alpha_blending(buf, premultiplied=False)
                    savename = osp.join(saved, f'{merged_name}.png')
                    Image.fromarray(merged).save(savename)
        else:
            for rst, tag in zip(pipeline_output.images, head_tag_list):
                rst = smart_resize(rst, scale_size)[py1: py2, px1: px2]
                full = canvas.copy()
