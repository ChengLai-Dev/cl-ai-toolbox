...
1. 默认自动先 `continue`（把当前暂停状态释放），然后等待下一个断点事件
...
参数:
{
  "auto_continue": true,  // 等待前是否先 continue（当前暂停时设为 false 避免意外恢复执行）
...
**注意事项**：
- `wait_next_hit` 默认 `auto_continue: true`，会先 `continue` 再等待。如果当前已在暂停状态且**不希望先恢复执行**（如异常断点触发后想保持暂停），请传 `auto_continue: false`
